...
**Known, unchanged-by-this-plan race, noted for the record:** two `_finish_calculate` threads for the *same* `sid` (e.g. a silent preview's finish and a real calculate's finish landing close together) both write `DOWNLOAD_CACHE` and `client_state[sid]['current_token']` with no lock — whichever finishes last wins, same as today. This already existed before this change (the silent path already shared this exact bookkeeping with the real path); running the tails on separate threads narrows the arrival-order window slightly but doesn't introduce a new failure mode. Out of scope here — flagged for separate investigation if it's ever observed to matter (e.g. a stale download token after a rapid extrude-length tweak).
