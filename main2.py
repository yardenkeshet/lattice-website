...
def handle_calculate(data):
    ...
    logger.info(f"[CALC]   raw payload : {len(igs_bytes):,} bytes  binary={is_binary}")
    ...
    try:
        with open(igs_disk_path, 'wb') as f:
            f.write(igs_bytes)
        logger.info(f"[CALC] Saved IGS -> {igs_disk_path}")
...
def handle_convert_igs_to_stl(data):
    ...
    try:
        # Save incoming IGS text to a temp file
        igs_bytes = igs_text.encode('utf-8') if isinstance(igs_text, str) else igs_text
        igs_disk  = os.path.abspath(os.path.join(DATA_DIR, f"{sid}_preview.igs"))
        stl_disk  = os.path.abspath(os.path.join(DATA_DIR, f"{sid}_preview.stl"))
        ...
        if not os.path.exists(stl_disk):
            return emit('error', {'msg': 'IGS conversion produced no output'})

        with open(stl_disk, 'rb') as f:
            stl_content = f.read()

        gzipped = gzip.compress(stl_content)
        b64_str = base64.b64encode(gzipped).decode('utf-8')
        emit('result', {'stl_gz_b64': b64_str, 'kind': 'model_preview_stl'})
        print("IGS -> STL conversion succeeded.")
    except Exception as exc:
        logger.exception(f"[IGS2STL] {exc}")
        emit('error', {'msg': f'Internal server error: {exc}'})
