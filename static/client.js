...
let renderer, scene, camera, perspectiveCamera, orthographicCamera, controls;
function init() {
...
function updateTextToCamera() {
  ...
  if (camera.isPerspectiveCamera) {
    textSprite.position.set(0, 0.45, -1);
    textSprite.scale.set(0.5, 0.15, 1);
  } else {
  console.log("updateTextToCamera")
    // In Orthographic, Z doesn't change size, but it must be within the frustum.
    // Use a very small Z to keep it "on top" of the 3D model.
    textSprite.position.set(0, 20, -0.1);

    // Scale for Ortho depends on the zoom level
    const s = 2 / camera.zoom;
    textSprite.scale.set(s, s * 0.3, 1);
  }
}
// ------------ End HUD
...
function fitCameraToMesh(mesh) {
    ...
    // Change padding to 1.2 to bring the camera closer (was 3.0)
    const padding = 2.5;
    ...
    if (camera.isPerspectiveCamera) {
        ...
        // ✅ Correct Orthographic Zoom
        // Instead of a hardcoded 100, use the actual frustum height
        const frustumHeight = camera.top - camera.bottom;
        camera.zoom = frustumHeight / (maxDim * padding);

        // Ensure the camera is far enough back to see the whole object
...
cameraModeToggle.addEventListener('click', () => {
    ...
    //if (textSprite) camera.add(textSprite);
